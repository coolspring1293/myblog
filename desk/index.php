<?php
    ...
    if (!empty($_REQUEST['username'])) {
        ...
        $p->content()->add('<h2> If it is not your user name. Please click the "log in" button.</h2>'); 
    } 
    ...
    {
        $p->content()->add('<h2>Please Log in first!</h2>');
    }
    
    $cp = $p->addContent(new jqmCollapsible(), true);
    $cp->title('Notices')->collapsed(true)->add('<p>图书馆的书被收走了😨. by liuw53</p><p>有什么需要反馈的，New Post写下来就好。</p>')->theme('a');
