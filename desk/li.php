...
<script>  
    $(document).ready(function() {  
        //alert('b');  
       setTimeout(function(){$('#use_coo').submit();},10);  
    });  
</script>  
 ...
 if(isset($_REQUEST['username'])) {
    ...
    $tmm = $row['ct'];
 	if ($tmm == 1) 
 	{
        ...
        setcookie('UserName', $un, time()+60*60*48);
        echo '<form id="use_coo" action="index.php" method="post"> <input type="text" name="username" value=" '. $_REQUEST['username'] .' "/></form>';
        if (!empty($_COOKIE['UserName'])) {
        	
            $p->addContent('<p style="padding:20px;">' . $_COOKIE['UserName'] . '</p>');
        }
        else {
        	$p->addContent('<p style="padding:20px;">No cookies.</p>');
        }
